...
document.getElementById('schedule-form').addEventListener('submit', async function(event) {
    ...
    if (!tutorSelect.value || !date || !time) {
        ...
        return;  // Prevents the form from proceeding if fields are incomplete
    }

    try {
        // Your scheduling logic and Google Calendar API integration
        alert("Session scheduled successfully!");
        window.location.href = 'sessions.html';  // Redirects only after scheduling
    } catch (error) {
